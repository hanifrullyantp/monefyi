...
export default function RightPanel({ projectId, onOpenRap, onOpenProgress }: Props) {
  ...
  return (
    <aside className="hidden xl:flex flex-col w-72 shrink-0 shell-right-panel overflow-y-auto">
      <section className="p-4">
        <h3 className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5 mb-3">
          <Zap className="w-3.5 h-3.5 text-emerald-600" /> Aksi Cepat
        </h3>
        <div className="grid grid-cols-2 gap-2">
          {QUICK_ACTIONS.map(a => (
            <button
              ...
              className="surface-card surface-card-hover flex flex-col items-center gap-1.5 p-3 rounded-2xl transition-all text-center"
            >
      ...
      <div className="surface-divider mx-4" />
      ...
      <section className="p-4">
        ...
        <div className="space-y-2">
          {alerts.length === 0 ? (
            ...
            <div key={i} className="flex gap-2 p-3 rounded-2xl bg-amber-50/80 text-xs shadow-sm">
      ...
      <section className="p-4">
        <h3 className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5 mb-3">
          <History className="w-3.5 h-3.5 text-emerald-600" /> Aktivitas Terakhir
        </h3>
        <div className="space-y-1">
          {recentActivity.map(a => (
            <button
              ...
              className="w-full text-left p-2.5 rounded-xl hover:bg-emerald-50/50 text-xs transition-colors"
            >
