...
export default function EstimationProjectLinkCoachmark({ userId }: Props) {
  ...
  useEffect(() => {
    try {
      setVisible(localStorage.getItem(storageKey(userId)) !== 'true');
    } catch {
      setVisible(true);
    }
  }, [userId]);
  ...
  return (
    <div className="relative shrink-0">
      ...
      <div className="absolute left-0 top-full mt-2 z-50 w-64 p-3 rounded-xl bg-white text-slate-700 text-xs leading-relaxed shadow-xl border border-slate-200">
        <button
          type="button"
          onClick={dismiss}
