  ...
  return (
    <div ref={ref} className="relative bg-white rounded-xl p-3 border text-center">
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
        className="absolute top-2 right-2 p-1 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50"
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full mt-1 z-30 p-3 bg-slate-900 text-white text-left rounded-xl shadow-xl text-xs leading-relaxed">
          ...
          <p className="font-mono text-indigo-200 text-[10px] mb-1">{metric.formula}</p>
