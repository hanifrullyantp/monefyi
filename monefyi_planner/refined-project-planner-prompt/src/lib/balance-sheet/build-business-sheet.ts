// =====================================================
// Build neraca umum (bisnis) from APP_DATA.business
// =====================================================

import type { APP_DATA } from "../mock-data";
import type { BalanceLine, BalanceSheet } from "./types";

type Business = typeof APP_DATA.business;

/**
 * Build business balance sheet with consistent line items.
 * Aktiva = Kas + Piutang + Persediaan + Aset Tetap + Prabayar
 * Pasiva = Hutang | Ekuitas = Modal + Laba Ditahan
 */
export function buildBusinessSheet(business: Business): BalanceSheet {
  const piutang = business.piutangList.reduce((s, p) => s + p.amount, 0);
  const persediaan = business.assets.reduce((s, a) => s + a.value, 0);
  const prabayar = Number((business as Business & { prabayar?: number }).prabayar ?? 0);
