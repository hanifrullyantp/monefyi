// =====================================================
// Map Monefyi planner_* rows → refined-project-planner APP_DATA shape
// =====================================================

import type { APP_DATA } from "./mock-data";

type AppData = typeof APP_DATA;
type AppProject = AppData["projects"][number];
...
export type PlannerFinanceAccountRow = {
  id: string;
  name: string;
  type: string;
  category: string;
...
/** Aggregate realized qty and amount per RAP line (matches costService.aggregateCostByRapItem). */
...
function mapRapItem(
  row: PlannerRapRow,
  idx: number,
  actualByRapId: Map<string, RapActualAgg>
...
/**
 * Build one APP project from planner rows.
 */
export function mapPlannerProject(input: {
  ...
  const totalRap = [...materials, ...workers].reduce(
    (s, item) => s + num(item.rapTotal),
    0
  );
  const bahanPlan = materials.reduce((s, item) => s + num(item.rapTotal), 0);
  const tukangPlan = workers.reduce((s, item) => s + num(item.rapTotal), 0);
  const bahanActual = materials.reduce((s, item) => s + num(item.total), 0);
  const tukangActual = workers.reduce((s, item) => s + num(item.total), 0);
  ...
  const hutangPiutang: AppProject["hutangPiutang"] = [];
    if (hutang > 0) {
  ...
  return {
    id: p.id as unknown as AppProject["id"],
    name: p.name,
    ...
    rap: {
      ...
      realisasi: spent,
      estLaba,
      materials: materials as AppProject["rap"]["materials"],
      workers: workers as AppProject["rap"]["workers"],
    },
    budget: {
...
/**
 * Assemble full app payload from planner tables.
 */
export function assemblePlannerPayload(input: {
  org: PlannerOrgRow;
  ...
  incomes: PlannerIncomeRow[];
  workItems: PlannerWorkRow[];
  accounts: PlannerFinanceAccountRow[];
  mockFallback: AppData;
}): AppData {
  ...
  const rapMap = byProject(input.rapItems);
  const costMap = byProject(input.costs);
  const incomeMap = byProject(input.incomes);
  const workMap = byProject(input.workItems);

  const projects = input.projects.map((project) =>
    mapPlannerProject({
      project,
      rapItems: rapMap.get(project.id) || [],
      costs: costMap.get(project.id) || [],
      incomes: incomeMap.get(project.id) || [],
      workItems: workMap.get(project.id) || [],
    })
  );

  const kasAccounts = input.accounts
    .filter((a) => a.type === "kas")
    .map((a, idx) => ({
      id: idx + 1,
      name: a.name,
  ...
  return {
    projects,
    database: input.mockFallback.database,
    business: {
      ...input.mockFallback.business,
      name: input.org.name,
      accounts: kasAccounts.length ? kasAccounts : input.mockFallback.business.accounts,
      totalKas: kasAccounts.length ? totalKas : input.mockFallback.business.totalKas,
    },
  };
}
