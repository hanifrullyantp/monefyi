...
import { storiesData } from '../data/testimonials-data';
...
export function useSiteSettings() {
  ...
  // Minimal logic to prevent crashes
  const settings = savedSettings || INITIAL_SETTINGS;

  const save = useCallback(() => {
    // No-op if we're not using draft, or we can just say alert
  ...
  const setSettings = useCallback((val: any) => {
