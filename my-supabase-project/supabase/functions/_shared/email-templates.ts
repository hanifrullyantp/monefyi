...
export function welcomeOwnerHtml(orgName: string): string {
  return `<!DOCTYPE html><html><body style="font-family:sans-serif;padding:24px">
<h1>Selamat datang, Owner!</h1>
<p>Organisasi <strong>${orgName}</strong> siap digunakan. Mulai undang tim Anda.</p>
</body></html>`;
}

export function joinRequestHtml(params: { requesterName: string; requesterEmail: string; orgName: string; message?: string }): string {
  return `<!DOCTYPE html><html><body style="font-family:sans-serif;padding:24px">
<h1>Permintaan bergabung</h1>
<p><strong>${params.requesterName}</strong> (${params.requesterEmail}) ingin bergabung ke ${params.orgName}.</p>
${params.message ? `<p>Pesan: ${params.message}</p>` : ""}
<p>Buka aplikasi untuk approve/reject.</p>
</body></html>`;
}
...
export function welcomeMemberHtml(orgName: string): string {
  return `<!DOCTYPE html><html><body style="font-family:sans-serif;padding:24px">
<h1>Selamat datang!</h1>
<p>Anda bergabung dengan <strong>${orgName}</strong>. Login untuk memulai.</p>
</body></html>`;
}
