export function invitationEmailHtml(params: {
  ...
  const { orgName, role, inviterName, joinUrl, personalMessage, brandColor = "#059669" } = params;
...
export function welcomeOwnerHtml(orgName: string, appUrl: string): string {
  return `<!DOCTYPE html><html><body style="font-family:sans-serif;background:#f8fafc;padding:24px">
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;border:1px solid #e2e8f0">
  <h1 style="color:#059669;margin:0 0 8px">Selamat datang, Owner!</h1>
  <p>Organisasi <strong>${orgName}</strong> siap digunakan di Monefyi Planner.</p>
  <a href="${appUrl}/onboarding/owner" style="display:inline-block;background:#059669;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold;margin:16px 0">Lanjutkan onboarding</a>
</div></body></html>`;
}
...
export function joinRequestHtml(params: {
  ...
  return `<!DOCTYPE html><html><body style="font-family:sans-serif;background:#f8fafc;padding:24px">
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;border:1px solid #e2e8f0">
  <h1 style="color:#059669;margin:0 0 8px">Permintaan bergabung</h1>
  ...
  <a href="${params.reviewUrl}" style="display:inline-block;background:#059669;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold;margin:16px 0">Tinjau permintaan</a>
</div></body></html>`;
}
...
export function authActionEmailHtml(params: {
  ...
  return `<!DOCTYPE html><html><body style="font-family:sans-serif;background:#f8fafc;padding:24px">
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;border:1px solid #e2e8f0">
  <p style="color:#059669;font-weight:bold;margin:0 0 8px">Monefyi Planner</p>
  <h1 style="margin:0 0 16px;font-size:22px">${params.title}</h1>
  <p style="color:#475569;margin:0 0 24px">${params.body}</p>
  <a href="${params.actionUrl}" style="display:inline-block;background:#059669;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold">${params.actionLabel}</a>
...
export function welcomeMemberHtml(orgName: string, appUrl: string): string {
  return `<!DOCTYPE html><html><body style="font-family:sans-serif;background:#f8fafc;padding:24px">
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;border:1px solid #e2e8f0">
  <h1 style="color:#059669;margin:0 0 8px">Selamat datang!</h1>
  <p>Anda bergabung dengan <strong>${orgName}</strong>.</p>
  <a href="${appUrl}/login" style="display:inline-block;background:#059669;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold;margin:16px 0">Masuk ke Monefyi</a>
</div></body></html>`;
}
